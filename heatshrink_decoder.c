...
static unsigned char cHsDecBuf[(1 << HEATSHRINK_STATIC_LOOKAHEAD_BITS) + HEATSHRINK_STATIC_INPUT_BUFFER_SIZE + \
...
heatshrink_decoder *heatshrink_decoder_alloc(uint16_t input_buffer_size,
    ...
    heatshrink_decoder *hsd = (heatshrink_decoder *)&cHsDecBuf[0];
    if (hsd == NULL) { return NULL; }
...
void heatshrink_decoder_free(heatshrink_decoder *hsd) {
	/* Do nothing for static allocation */
}
#endif
